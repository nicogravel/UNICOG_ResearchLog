...
An important step in using functional MRI is the design of stimuli to target specific cortical regions and functions. Computational neuroimaging of the human visual cortex often relies on standard retinotopic paradigms that reflect the structure of the visual cortex (*e.g.*, rotating wedges, expanding rings, drifting bars) {cite:p}`Wandell_2007, Schira_2010`. These types of stimuli enable the mapping of retinotopic cortical areas using methods such as population receptive field (pRF) modeling. A complementary approach, connective field (CF) modeling {cite:p}`Haak_2013`, relies on the cortical responses in a source area to predict the retinotopic organization of visual cortex based on activity in a target area. Although CF modeling has proven useful in the study of human visual cortical areas even in the absence of visual stimuli {cite:p}`Gravel_2014`, a standardization of the method is lacking. Here we propose a validation framework for CF modeling.
